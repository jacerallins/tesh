...
export function VoicePanel({ voice, snapshot }: VoicePanelProps): ReactElement {
  ...
  return <section className="voice-panel" aria-label="Development voice controls">
    ...
    <div className="voice-actions">
      ...
      <button type="button" onClick={() => void voice.startListening()} disabled={snapshot.microphone === 'active'}>Start listening</button>
      <button type="button" onClick={() => void voice.stopListening()} disabled={snapshot.microphone === 'inactive'}>Stop listening</button>
    ...
    <div className="voice-readout">
      ...
      <div><span className="panel-label">Input</span><strong>{snapshot.audio.inputDevice || 'Not active'}</strong></div>
    </div>
    ...
    <p className="preview-copy">Recognition provider: Browser SpeechRecognition. {voice.getSnapshot().recognition === 'error' ? 'The provider is unavailable or returned an error.' : 'Network behavior depends on the browser speech service; no audio is stored or uploaded by Tesh.'}</p>
