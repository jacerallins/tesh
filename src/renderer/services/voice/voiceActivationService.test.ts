...
describe('VoiceActivationService', () => {
  ...
  it('auto-enrolls the development identity and listens after a simulated wake', async () => {
    const { activation, wake, voice } = createActivation();
    await activation.start();
    expect(activation.getSnapshot().enrollment).toBe('ENROLLED');
    wake.simulateDetection();
    await vi.waitFor(() => expect(activation.getSnapshot().phase).toBe('LISTENING'));
    expect(activation.getSnapshot().lastResult).toBe('VERIFIED');
    expect(voice.starts).toBe(1);
    await activation.stop();
  });

  it('creates a verified session after explicit mock enrollment and pass', async () => {
  ...
  it('enforces failed-attempt backoff and expires verified sessions', async () => {
    const { activation, speaker, wake, voice } = createActivation(500);
    await activation.start();
