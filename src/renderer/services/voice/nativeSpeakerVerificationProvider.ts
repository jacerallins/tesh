...
export class NativeSpeakerVerificationProvider implements SpeakerVerificationProvider {
  ...
  async verify(): Promise<VerificationAttempt> {
    ...
    const attempt = await bridge.verifySpeaker();
    return { result: attempt.result, confidence: attempt.confidence, liveness: attempt.liveness, method: attempt.method };
  }

  async removeEnrollment(): Promise<void> { this.enrolled = false; }
  async getStatus(): Promise<PrimaryUserIdentity | undefined> {
    const configured = await window.tesh?.voice?.isSpeakerConfigured();
    if (!configured || !this.enrolled) return undefined;
    const now = new Date().toISOString();
    return { id: 'primary-user', displayName: 'Primary user', enrollmentStatus: 'ENROLLED', createdAt: now, updatedAt: now, verificationVersion: 'sherpa-onnx-v1' };
  }
