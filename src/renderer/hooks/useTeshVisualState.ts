...
export interface TeshVisualStateController {
  ...
  beginListening: () => TransitionResult;
  stopListening: () => TransitionResult;
  beginThinking: () => TransitionResult;
  beginProcessing: (task?: string) => TransitionResult;
  beginSpeaking: (audio?: AudioState) => TransitionResult;
  ...
  simulateState: (state: TeshVisualState) => TransitionResult;
  voice: VoiceController;
  voiceSnapshot: ReturnType<VoiceController['getSnapshot']>;
  activation: VoiceActivationService;
  activationSnapshot: ReturnType<VoiceActivationService['getSnapshot']>;
}

export function useTeshVisualState(initialState: TeshVisualState = 'idle'): TeshVisualStateController {
  const [engine] = useState(() => {
    const instance = new TeshInteractionEngine();
    if (initialState !== 'idle') instance.simulateState(initialState);
    return instance;
  });
