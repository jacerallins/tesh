...
export interface NativeSpeakerVerificationOptions {
  executable: string;
  script: string;
  model: string;
  profile: string;
...
export class NativeSpeakerVerificationService {
  ...
  isConfigured(): boolean { return Boolean(this.options.script && this.options.model && this.options.profile); }
  isEnrolled(): boolean { return this.isConfigured() && existsSync(this.options.profile); }

  async enroll(samples = 3): Promise<void> {
    if (!this.isConfigured()) throw new Error('Native speaker verification is not configured.');
  ...
  async clearEnrollment(): Promise<void> {
    if (existsSync(this.options.profile)) unlinkSync(this.options.profile);
  }
