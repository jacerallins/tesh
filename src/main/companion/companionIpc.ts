...
const permissions: readonly CompanionPermission[] = ['COMPANION_VIEW_STATUS','COMPANION_SEND_COMMAND','COMPANION_RECEIVE_NOTIFICATIONS','COMPANION_SYNC_MEMORY','COMPANION_SYNC_CONVERSATIONS','COMPANION_SYNC_ROUTINES','COMPANION_CHAT'];
...
export function registerCompanionIpc(service:CompanionService,enabled:boolean):void{if(!enabled)return;ipcMain.handle('companion:snapshot',()=>service.getSnapshot());ipcMain.handle('companion:pairing-create',()=>service.createPairingChallenge());ipcMain.handle('companion:pair',(_e,input:unknown)=>service.pair(validatePairInput(input)));ipcMain.handle('companion:disconnect',(_e,id:unknown)=>service.disconnect(text(id,'Device ID',80)));ipcMain.handle('companion:revoke',(_e,id:unknown)=>service.revoke(text(id,'Device ID',80)));ipcMain.handle('companion:permission-grant',(_e,id:unknown,p:unknown)=>service.grantPermission(text(id,'Device ID',80),validateCompanionPermission(p)));}
