...
export class ConversationService implements AIConversationBridge {
  ...
  async send(conversationId: string, content: string): Promise<ConversationSnapshot> {
    const conversation = this.get(conversationId); if (!content.trim() || content.length > 10000) throw new Error('Message content is invalid.');
    const user: AIMessage = { id: randomUUID(), role: 'USER', content: content.trim(), timestamp: new Date().toISOString() }; conversation.messages.push(user); conversation.status = 'GENERATING'; conversation.lastError = undefined;
    ...
    } catch (error) { const providerError = error as AIProviderError; conversation.status = 'ERROR'; conversation.lastError = providerError.code ?? 'AI_REQUEST_FAILED'; return conversation; } finally { this.controllers.delete(conversationId); }
  }
