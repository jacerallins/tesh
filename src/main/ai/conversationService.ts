...
export class ConversationService implements AIConversationBridge {
  ...
  constructor(private readonly provider: AIProvider, private readonly tools: ToolExecutor, private readonly retrieveMemory: (query: string) => Promise<Memory[]> = async () => [], private readonly repository?: ConversationRepository) {
    if (repository) for (const conversation of repository.list(100)) this.conversations.set(conversation.id, conversation);
  }
  ...
  async send(conversationId: string, content: string): Promise<ConversationSnapshot> {
    const conversation = this.get(conversationId); if (!content.trim() || content.length > 10000) throw new Error('Message content is invalid.'); if (this.busyConversations.has(conversationId)) throw new Error('Conversation is already processing another request.'); this.busyConversations.add(conversationId);
    ...
    try {
      ...
      const response = await this.provider.generate(buildContext(conversation.messages, TESH_SYSTEM_PROMPT, memoryContext), [...toolDefinitions], controller.signal); conversation.lastLatencyMs = Date.now() - started;
      ...
      const followUp = await this.provider.generate(buildContext(conversation.messages, TESH_SYSTEM_PROMPT, memoryContext), [...toolDefinitions], controller.signal); if (followUp.toolRequest) { conversation.status = 'ERROR'; conversation.lastError = 'AI_INVALID_RESPONSE'; this.persist(conversation); return conversation; }
      return this.completeResponse(conversation, followUp.content);
    } catch (error) { const providerError = error as AIProviderError; conversation.status = 'ERROR'; conversation.lastError = providerError.code ?? 'AI_REQUEST_FAILED'; this.persist(conversation); return conversation; }
    finally { this.controllers.delete(conversationId); this.busyConversations.delete(conversationId); }
  }
  async cancel(conversationId: string): Promise<void> { this.controllers.get(conversationId)?.abort(); this.provider.cancel(); const conversation = this.get(conversationId); conversation.status = 'ERROR'; conversation.lastError = 'AI_CANCELLED'; this.persist(conversation); }
