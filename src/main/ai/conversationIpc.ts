...
export function registerConversationIpc(service: ConversationService): void {
  ...
  ipcMain.handle('conversation:list', (_event, limit?: unknown) => service.list(typeof limit === 'number' ? limit : 50));
  ipcMain.handle('conversation:get', (_event, conversationId: unknown) => { if (typeof conversationId !== 'string') throw new Error('Conversation ID is invalid.'); return service.get(conversationId); });
