...
export class ConversationRepository {
  ...
  save(snapshot: ConversationSnapshot): void { const createdAt = snapshot.createdAt ?? new Date().toISOString(); const updatedAt = new Date().toISOString(); snapshot.createdAt = createdAt; snapshot.updatedAt = updatedAt; this.db.prepare('INSERT INTO conversations (id, created_at, updated_at, snapshot_json) VALUES (?, ?, ?, ?) ON CONFLICT(id) DO UPDATE SET updated_at=excluded.updated_at, snapshot_json=excluded.snapshot_json').run(snapshot.id, createdAt, updatedAt, JSON.stringify(snapshot)); }
