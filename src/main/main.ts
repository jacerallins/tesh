...
ipcMain.handle('voice:wake-start', async (_event, phrase: string) => { await nativeWake.start(phrase); nativeWake.onDetected(() => { showAssistant(); dashboardWindow?.webContents.send('voice:wake-detected'); }); });
