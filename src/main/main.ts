...
function createOverlayWindow(): BrowserWindow {
  ...
  if (isDevelopment) void window.loadURL('http://localhost:5173/?overlay=1');
  else void window.loadFile(path.join(__dirname, '../../dist/index.html'), { query: { overlay: '1' } });
...
app.whenReady().then(() => {
  ...
  if (process.env.TESH_COMPANION_TLS_CERT && process.env.TESH_COMPANION_TLS_KEY) {
    const companionServer = new TlsCompanionServer(companion, audit);
    void companionServer.start({ certificate: process.env.TESH_COMPANION_TLS_CERT, privateKey: process.env.TESH_COMPANION_TLS_KEY, host: process.env.TESH_COMPANION_BIND_HOST ?? '127.0.0.1', port: Number(process.env.TESH_COMPANION_PORT ?? 0) });
  }
  createTray();
