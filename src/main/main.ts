...
ipcMain.handle('voice:speaker-configured', () => nativeSpeaker.isConfigured()); ipcMain.handle('voice:speaker-enrolled', () => nativeSpeaker.isEnrolled());
ipcMain.handle('voice:speaker-enroll', (_event, prompt?: string, sampleCount?: number) => nativeSpeaker.enroll(prompt ?? '', sampleCount ?? 1)); ipcMain.handle('voice:speaker-verify', () => nativeSpeaker.verify()); ipcMain.handle('voice:speaker-clear', () => nativeSpeaker.clearEnrollment());
...
app.whenReady().then(() => {
  ...
  const routines = new RoutineService(memoryDatabase.connection, audit, async (command) => developmentVerifiedSession && permissionService.authorizeAction({ capabilityId: command === 'FILE_READ' ? 'file.read' : 'system.diagnostics', resource: 'routine' }).result === 'ALLOWED'); registerRoutineIpc(routines);
