...
app.whenReady().then(() => {
  ...
  const files = new FileService(permissionService, () => developmentVerifiedSession, (event, resource) => audit.record(event, resource)); const system = new SystemService(app.getVersion(), permissionService, () => developmentVerifiedSession, (event) => audit.record(event)); registerSystemIpc(files, system, (verified) => { if (isDevelopment) developmentVerifiedSession = verified; });
  ...
  const companion = new CompanionService(memoryDatabase.connection, audit, (command) => command === 'get_tesh_status'); registerCompanionIpc(companion, isDevelopment);
